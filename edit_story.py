...
def edit_story(unedited_story_filename = None, max_number_of_attempts = 3, B = 3):
    ...
    for unedited_story_filename in unedited_story_filenames:
        ...
        # each entry of the chunks_editing_data will be an instance of chunk_editing_data, which is itself a list of "version objects" that corresponds to the iterative edits of a given chunk. in turn, a "version object" has:
            # text: a string
            # triggers: an object containing:
                # any_numbers: bool
                # numbers_appearing: list of strings
                # any_roman_numerals: bool
                # roman_numerals_appearing: list of strings
                ...
                # any_overused_words_to_remove: bool, namely: the `any` of all the above `try_to_remove` bools
        ...
        for unedited_story_chunk in unedited_story_chunks:
            ...
            # initialize the chunk_editing_data, which will be appended to chunks_editing_data later.
            chunk_editing_data = []
            ...
            # if we hit the last pass, we will _not_ attempt a rewrite. this organization is just so that the code for creating the version_object is recycled.
            for index in range(max_number_of_attempts + 1):

                # create a new version-object.
                version_object = {
                    "text": chunk_to_check,
                    # the following property will be populated in the `for` loop just below. it will be an object, whose keys are overused words and whose values are objects.
                    "overused_words_data": {},
                }
                ...
                any_overused_words_to_remove = any([overused_word_object["try_to_remove"] for overused_word_object in version_object["overused_words_data"].values()])
                version_object["any_overused_words_to_remove"] = any_overused_words_to_remove
                any_triggers = any([version_object["any_numbers"], version_object["any_roman_numerals"], version_object["any_overused_words_to_remove"]])
                ...
                if any_triggers:
                    if index < max_number_of_attempts:
                        ...
                        # probably just break here?
                        x=5
                else:
                    # record stats, then break.
                    # what stats? the number of rewrites is evident from the length of the list of version-objects, and the success/failure is evident from the any_triggers boolean on the last of them. so maybe actually this is not needed.
                    x=5
