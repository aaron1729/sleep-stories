...
this repo provides an end-to-end pipeline for generating travel-based sleep stories (with a chosen travel destination) and accompanying images/videos in a chosen artistic style, as illustrated in the [pipeline flowchart](./sleep_stories_pipeline.pdf).

<!--
below is a table of contents, generated by the `markdown-toc` package.
to regenerate it, just run `markdown-toc -i README.md` in the terminal!
-->
...
- [pipeline](#pipeline)
  * [miscellaneous info](#miscellaneous-info)
    ...
    + [miscellaneous to-do (not including stuff below)](#miscellaneous-to-do-not-including-stuff-below)
  * [get_stops.py](#get_stopspy)
  * [write_story.py](#write_storypy)
  * [edit_story.py](#edit_storypy)
  * [make_cues.py](#make_cuespy)
  * [get_phoneticizations.py](#get_phoneticizationspy)
  * [make_kt.py](#make_ktpy)
  * [get_art_style_description.py](#get_art_style_descriptionpy)
  * [get_dalle_prompts.py](#get_dalle_promptspy)
  * [get_pix.py](#get_pixpy)
  * [[making a video in imovie from images]](#making-a-video-in-imovie-from-images)
  * [rotate_vids.py](#rotate_vidspy)
  * [word_usage_stats.py](#word_usage_statspy)
...
the following should be installed by `pip install`. a (probably incomplete) list of modules used in this repo is:
...
* `subprocess` (for running commands on the command line).
also, the node module `markdown-toc` is used for making a table of contents for this markdown document.

## instructions

trace through the pipeline in a topological ordering. then, check for any instances of the words "knowledge" and "cutoff" and "update", to ensure that in its responses chatGPT didn't accidentally refer e.g. to its "knowledge cutoff" or to a "recent update".
...
### [making a video in imovie from images]

here are the steps i've been following (in jan 2024). (previously, i animated the images using runway, so the workflow was slightly different.)

1. rotate all images left (or do this in imovie).
1. drag & drop them all into imovie. (note that imovie _only_ works in landscape mode.)
1. reorder them as desired. (to view the images clearly, it may be helpful to set them into `fit` mode (as opposed to e.g. `ken burns` mode).)
1. select all, and in the upper-right panel change all of their durations to 16sec.
1. add cross-dissolve between all images. (after selecting an image, doing `command-T` adds this both before and after it.)
1. set all cross-dissolve lengths to 1.5sec.
1. manually change all ken burns start & end frames.

    i. try to keep these pretty steady -- don't let them move too quickly. this takes a little practice to get a sense of.

    ii. try to change up the direction of movement from one image to the next.

1. exit the project, naming it e.g. `vid-rotated_beijing_in_chinese-silk`.
1. export the file, and save it in `vids/yyyy-mm-dd/` (where `yyyy-mm-dd` is a datestamp).

### rotate_vids.py

to make videos, i've been stitching together images from dalle (possibly piped through runway to get video) in apple's imovie app. this _only_ supports landscape videos. i can attempt to rotate them (e.g. in apple's finder app), but apple's ecosystem saves this merely as metadata rather than actually rotating the video in some deeper sense. so, this script takes "apparently-rotated" videos (which were originally generated in such a way that they need to be rotated 90 degrees _clockwise_) and makes them into "truly-rotated" videos. it draws from `vids/yyyy-mm-dd/vids-rotated` and saves to `vids/yyyy-mm-dd/vids`, where `yyyy-mm-dd` is a datestamp specified in the code.
